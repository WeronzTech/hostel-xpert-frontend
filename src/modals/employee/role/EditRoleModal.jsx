...
import {editRoles, getAllRoles} from "../../../hooks/employee/useEmployee";
...
const routePermissions = [
  ...
  {label: "PAYROLL_VIEW", value: "PAYROLL_VIEW"},
  {label: "PAYROLL_MANAGE", value: "PAYROLL_MANAGE"},
...
const EditRoleModal = ({open, onClose, role}) => {
  ...
  const handleEditRole = useMutation({
    mutationFn: (data) => editRoles(role._id, data),
    onSuccess: () => {
  ...
  return (
    <>
      ...
      <Modal
        ...
        onOk={handleOk}
        onCancel={onClose}
        confirmLoading={handleEditRole.isPending}
        okText="Save Changes"
      >
        <Form form={form} layout="vertical" name="editRoleForm">
          ...
          <Form.Item name="reportTo" label="Reports To (Optional)">
            ...
            >
              {rolesData
                ?.filter((r) => r._id !== role?._id) // Prevent a role from reporting to itself
                .map((r) => (
