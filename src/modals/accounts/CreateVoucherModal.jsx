...
import {useSelector} from "react-redux";
import {useMutation, useQuery, useQueryClient} from "@tanstack/react-query";
import {addVoucher, getAllPettyCashes} from "../../hooks/accounts/useAccounts";
import {UserOutlined, ShopOutlined} from "../../icons";
...
const CreateVoucherModal = ({visible, onCancel, onSuccess}) => {
  ...
  // State for selected property and manager
  const [selectedPropertyId, setSelectedPropertyId] = useState(null);
  const [selectedManager, setSelectedManager] = useState(null);
  const [filteredManagers, setFilteredManagers] = useState([]);

  // Query for petty cashes
  const {
    data: pettyCashes = [],
    isLoading: pettyCashLoading,
    error: pettyCashError,
  } = useQuery({
    queryKey: ["pettyCashes", selectedPropertyId, user.userType],
    queryFn: () =>
      getAllPettyCashes({
    ...
    staleTime: 5 * 60 * 1000,
    // enabled: !!selectedPropertyId && !!visible,
  });

  // Filter managers with inHandAmount > 0
  useEffect(() => {
    if (pettyCashes.length > 0) {
      const managersWithCash = pettyCashes.filter((pc) => pc.inHandAmount > 0);
      setFilteredManagers(managersWithCash);

      // Reset manager selection when property changes
  ...
  const addVoucherMutation = useMutation({
    ...
    onSuccess: (data) => {
      ...
      // Invalidate petty cashes query to refresh data
      queryClient.invalidateQueries({
        queryKey: ["pettyCashes", selectedPropertyId],
      });
  ...
  const handleManagerChange = (value, option) => {
    const manager = filteredManagers.find((m) => m._id === value);
  ...
  const validateAmount = (_, value) => {
    ...
    // Validate against selected manager's inHandAmount
    if (selectedManager && numericValue > selectedManager.inHandAmount) {
      return Promise.reject(
        new Error(
          `Amount cannot exceed available cash of ₹${selectedManager.inHandAmount.toLocaleString()}`,
        ),
      );
    }
  ...
  // Show error if petty cash fetching fails
  if (pettyCashError) {
    return (
      <Modal
        title="Create Voucher"
        ...
        >
          ...
          <Card size="small" style={{marginBottom: 16}}>
            ...
            >
              ...
              >
                {properties
                  .filter((prop) => prop._id !== null) // Filter out "All Properties" if needed
                  .map((property) => (
          ...
          {/* Manager Selection and Available Cash */}
          {selectedPropertyId && (
            <Card size="small" style={{marginBottom: 16}}>
              <Form.Item
                ...
                <Select
                  ...
                  notFoundContent={
                    pettyCashLoading ? (
                      ...
                      "No managers with available cash"
                    )
                  }
                >
                  {filteredManagers.map((manager) => (
                    <Option key={manager._id} value={manager._id}>
                      {manager.managerName} - ₹
                      {manager.inHandAmount.toLocaleString()}
                    </Option>
                  ))}
                </Select>
              </Form.Item>
              ...
              {selectedManager && (
                <div style={{marginTop: 8, textAlign: "center"}}>
                  <Statistic
                    title={`Available Cash with ${selectedManager.managerName}`}
                    value={selectedManager.inHandAmount}
                    precision={2}
                    valueStyle={{
                      color:
                        selectedManager.inHandAmount > 0
                          ? "#3f8600"
                          : "#cf1322",
                      fontSize: "20px",
                    }}
                  ...
                  {selectedManager.inHandAmount === 0 && (
                    <Alert
                      message="No available cash with this manager"
                      type="warning"
                      showIcon
                      style={{marginTop: 8}}
                    />
          ...
          {selectedManager && (
            <>
              <Divider plain>Voucher Details</Divider>
              <Card size="small">
                {/* Name Field */}
                <Form.Item
                  label="Recipient Name"
                ...
                <Form.Item
                  ...
                  help={
                    selectedManager
                      ? `Maximum allowed amount: ₹${selectedManager.inHandAmount.toLocaleString()}`
                      : null
                  }
                >
                  <InputNumber
                    ...
                    disabled={
                      !selectedManager || selectedManager.inHandAmount === 0
                    }
                    onChange={handleAmountChange}
                    onBlur={() => form.validateFields(["amount"])}
                  />
          ...
          >
            ...
            <Button
              ...
              htmlType="submit"
              loading={addVoucherMutation.isLoading}
              disabled={!selectedManager || selectedManager?.inHandAmount === 0}
            >
              Create Voucher
            </Button>
          </Form.Item>
